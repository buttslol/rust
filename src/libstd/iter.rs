...
pub trait Iterator<A> {
    ...
    /// ```
    #[inline]
    fn map<'r, B>(self, f: |A| -> B) -> Map<'r, A, B, Self> {
        Map{iter: self, f: f}
    }
    ...
    /// ```
    #[inline]
    fn filter<'r>(self, predicate: |&A| -> bool) -> Filter<'r, A, Self> {
    ...
    /// ```
    #[inline]
    fn filter_map<'r, B>(self, f: |A| -> Option<B>) -> FilterMap<'r, A, B, Self> {
    ...
    /// ```
    #[inline]
    fn skip_while<'r>(self, predicate: |&A| -> bool) -> SkipWhile<'r, A, Self> {
    ...
    /// ```
    #[inline]
    fn take_while<'r>(self, predicate: |&A| -> bool) -> TakeWhile<'r, A, Self> {
    ...
    /// ```
    #[inline]
    fn scan<'r, St, B>(self, initial_state: St, f: |&mut St, A| -> Option<B>)
        -> Scan<'r, A, B, Self, St> {
    ...
    /// ```
    #[inline]
    fn flat_map<'r, B, U: Iterator<B>>(self, f: |A| -> U)
        -> FlatMap<'r, A, Self, U> {
    ...
    /// ```
    #[inline]
    fn inspect<'r>(self, f: |&A|) -> Inspect<'r, A, Self> {
...
impl<'a, A, St> Unfold<'a, A, St> {
    ...
    #[inline]
    pub fn new<'a>(initial_state: St, f: |&mut St| -> Option<A>)
               -> Unfold<'a, A, St> {
        Unfold {
            f: f,
            state: initial_state
